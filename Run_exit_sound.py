...
def main():
	...
	while True:
		...
		text = "High risk: {}".format(len(serious))
		cv2.putText(frame, text, (10, frame.shape[0] - 55),
			cv2.FONT_HERSHEY_SIMPLEX, 0.70, (0, 0, 255), 2)
		text1 = "Moderate risk: {}".format(len(abnormal))
		cv2.putText(frame, text1, (10, frame.shape[0] - 25),
			cv2.FONT_HERSHEY_SIMPLEX, 0.70, (0, 255, 255), 2)
		if len(serious) >= config.Threshold:
			cv2.putText(frame, "Warning: Please keep your distance!", (10, frame.shape[0] - 80),
		...
		if args["display"] > 0:
			cv2.imshow("Real-Time Social DIstance Detector", frame)
